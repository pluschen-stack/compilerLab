...
#define STACK_INC_DEPTH(s) \
    if (s)                 \
        s->stackDepth++; //栈增长
#define STACK_DEC_DEPTH(s) \
    if (s)                 \
        s->stackDepth--;
#define GET_STACK_HEAD(s) s ? s->stackArray[s->stackDepth] : NULL
#define SET_STACK_HEAD(s, i)              \
    if (s)                                \
    {                                     \
        s->stackArray[s->stackDepth] = i; \
    }
#define GET_HASH_HEAD(h, c) h ? h->hashArray[c] : NULL
#define SET_HASH_HEAD(h, c, i) \
    if (h)                     \
    {                          \
        h->hashArray[c] = i;   \
    }
#define SET_FEILDLIST_NAME(f, n) \
    if (f->name)                 \
    {                            \
        FREE(f->name);           \
        f->name = newString(n);  \
